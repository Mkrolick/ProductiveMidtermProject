...
int ReadNum(FILE *fp) {
  ...
  int ch;  // use int, which is in fact return type of fgetc to avoid casting of
           // 255 to -1
...
Image *ReadPPM(FILE *fp) {
  ...
  // read in columns
  im->cols =
      ReadNum(fp);  // NOTE: cols, then rows (i.e. X size followed by Y size)
  // read in rows
  im->rows = ReadNum(fp);
  ...
  if (!im->data) {
    fprintf(stderr,
            "Error:ppm_io - failed to allocate memory for image pixels!\n");
    free(im);
    return NULL;
  }
  ...
  int num_pixels_read =
      (int)fread(im->data, sizeof(Pixel), (im->rows) * (im->cols), fp);
  if (num_pixels_read != (im->rows) * (im->cols)) {
    fprintf(stderr,
            "Error:ppm_io - failed to read data from file with size %d (read "
            "%d)!\n",
...
int WritePPM(FILE *fp, const Image *im) {
  ...
  int num_pixels_written =
      (int)fwrite(im->data, sizeof(Pixel), (im->rows) * (im->cols), fp);
